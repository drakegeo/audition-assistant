...
export default function SetupPage({ params }: { params: Promise<{ scriptId: string }> }) {
  const [scriptId, setScriptId] = useState<string | null>(null);

  useEffect(() => {
    params.then(({ scriptId: id }) => setScriptId(id));
  }, [params]);
  ...
  return (
    <AuthGuard>
      <SetupInner scriptId={scriptId} />
    </AuthGuard>
  );
}

function SetupInner({ scriptId }: { scriptId: string }) {
  ...
  useEffect(() => {
    loadKokoro()
      ...
      .catch(() => {/* will fall back to Web Speech in rehearsal */});
  }, []);
        ...
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          <ul className="divide-y">
            {otherChars.map((char) => {
              ...
              const isThisPreviewing = previewing === char.name;
              return (
